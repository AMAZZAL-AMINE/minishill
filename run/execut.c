...
/*   Updated: 2023/07/30 21:37:32 by mamazzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	char **content, t_minishell *mini)
{
	...
	tmp = find_cmd_path(shell->cmd + length_cmd(shell->cmd), \
		mini, shell);
	if (str_cmp(tmp, shell->cmd))
	{
		shell->args = join_two_dim_arr(shell->cmd + \
			length_cmd(shell->cmd), content);
