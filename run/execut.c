...
/*   Updated: 2023/07/20 16:52:13 by mamazzal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void run_simple_commande(int is_path, t_parsing *shell, char **content, t_minishell *mini) {
  ...
  // }
  if (is_path == 1) {
    if (execve(shell->cmd, join_two_dim_arr(shell->cmd + length_cmd(shell->cmd), content), mini->env_v) == -1)
      cmd_not_found(shell->cmd, mini);
  }else  {
    if (execve(find_cmd_path(shell->cmd, mini, shell), join_two_dim_arr(shell->cmd, content), mini->env_v) == -1)
      cmd_not_found(shell->cmd, mini);
  }
}
